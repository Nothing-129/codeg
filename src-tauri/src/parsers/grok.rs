...
use crate::parsers::{
    ...
    relocate_orphaned_tool_results, structurize_read_tool_output, truncate_str, AgentParser,
    ParseError,
};
...
impl GrokParser {
    ...
    fn build_summary(&self, session_dir: &Path, session_id: &str) -> Option<ConversationSummary> {
        ...
        if meta
            .cwd
            .as_deref()
            .is_some_and(crate::session_title::is_grok_title_scratch_cwd)
        {
            return None;
        }
    ...
    fn summary_from(
        ...
        // Prefer the first user line over Grok's English-biased
        // `generated_title`. Locale refine (and a user rename) live in the
        // DB and lock the row so this backfill cannot clobber them.
        let title = parsed
            .first_user_text
            .as_deref()
            .filter(|text| !crate::session_title::is_title_refine_prompt(text))
            .map(crate::session_title::heuristic_title)
            .filter(|t| !t.is_empty())
...
pub(crate) fn is_redundant_session_image_read(tool_name: &str, raw_input: Option<&Value>) -> bool {
...
mod tests {
    ...
    fn lists_session_with_metadata() {
        ...
        assert_eq!(s.title.as_deref(), Some("你会做什么"));
    ...
    #[test]
    fn prefers_first_user_line_over_generated_title() {
        let (_tmp, sessions) = fixture(SUMMARY, UPDATES);
        let parser = GrokParser::with_base_dir(sessions);
        let list = parser.list_conversations().unwrap();
        assert_eq!(list[0].title.as_deref(), Some("你会做什么"));
        assert_ne!(list[0].title.as_deref(), Some("Build the project"));
    }

    #[test]
    fn skips_title_scratch_sessions() {
        let summary = r#"{
            "info": {"id": "019f45e3-e1ef-7690-a29f-fe2554382b49", "cwd": "/Users/me/.codeg/grok-title-scratch"},
            "generated_title": "为下面这条用户消息起一个简短会话标题",
            "created_at": "2026-07-09T07:59:50.598122Z",
            "updated_at": "2026-07-09T08:02:09.789572Z"
        }"#;
        let (_tmp, sessions) = fixture(summary, UPDATES);
