...
async fn emit_conversation_update(
    ...
    match update {
        ...
        SessionUpdate::ToolCall(tc) => {
            ...
            {
                cb_state.grok_hidden_image_read_ids.insert(tool_call_id);
                return;
            }
        ...
        SessionUpdate::ToolCallUpdate(tcu) => {
            ...
            if cb_state.grok_hidden_image_read_ids.contains(&tool_call_id) {
                return;
            }
...
mod tests {
    ...
    /// Grok persists a native prompt image under its session `assets/`
